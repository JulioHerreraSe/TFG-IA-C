...
void initializeMLP(MLP *mlp, int inputSize, int hiddenSize, int outputSize) {
    ...
    for(int i = 0; i < hiddenSize; i++){
        for (int j = 0; j < inputSize; ++j) {
            printf("%lf ", mlp->hiddenLayer.weights[i][j]);
        }
        printf("\n");
    }
    */
    ...
    for(int i = 0; i < outputSize; i++){
        for (int j = 0; j < hiddenSize; ++j) {
            printf("%lf ", mlp->outputLayer.weights[i][j]);
        }
        printf("\n");
    }
    */
...
void train_MAE(MLP *mlp, double *input, double target, double learningRate) {
    int i, j;
    ...
    double mae_gradient = (error > 0) ? 1 : ((error < 0) ? -1 : 0); // Gradiente para MAE

    // Calcula el delta para la capa de salida usando MAE
    double outputDelta = mae_gradient;
    ...
    for (i = 0; i < HIDDEN_SIZE; i++) {
        hiddenDeltas[i] = 0;
        // Dado que MAE no es inherentemente diferenciable en el mismo sentido que MSE,
        // la propagación del error a través de las capas requiere atención especial.
        // Aquí simplemente propagamos el delta ajustado por los pesos y la derivada de la función de activación.
        for (j = 0; j < OUTPUT_SIZE; j++) {
            hiddenDeltas[i] += outputDelta * mlp->outputLayer.weights[j][i];
        }
        hiddenDeltas[i] *= ReLUDerivative(hiddenOutputs[i]); // Asume sigmoid como función de activación
    }
