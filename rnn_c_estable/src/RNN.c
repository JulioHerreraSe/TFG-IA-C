...
void initializeRNN(RNN *rnn, int inputSize, int hiddenSize, int outputSize) {
    ...
    for (i = 0; i < hiddenSize; i++) {
        ...
        for (j = 0; j < inputSize; j++) {
            rnn->hiddenLayer.weights[i][j] = generateXavierWeight(inputSize);
        }
        rnn->hiddenLayer.bias[i] = generateXavierWeight(inputSize);
        rnn->hiddenLayer.state[i] = 0;
        rnn->hiddenLayer.weightsState[i] = generateXavierWeight(inputSize);
    }
    ...
    rnn->outputLayer.state[0] = 0;

    for (i = 0; i < outputSize; i++) {
        rnn->outputLayer.weights[i] = (double *)malloc(hiddenSize * sizeof(double));
        for (j = 0; j < hiddenSize; j++) {
            rnn->outputLayer.weights[i][j] = generateXavierWeight(hiddenSize);
        }
        rnn->outputLayer.weightsState[i] = generateXavierWeight(hiddenSize);
        rnn->outputLayer.bias[i] = generateXavierWeight(hiddenSize);
    }
}
...
double forward(RNN *rnn, double *input, double hiddenOutput[HIDDEN_SIZE]) {
    ...
    rnn->outputLayer.state[OUTPUT_SIZE-1] = finalOutput; // Asumiendo OUTPUT_SIZE == 1.
...
double trainMSE(RNN *rnn, double *input, double target, double learningRate) {
    ...
    double error = target - predictedOutput;
    ...
    for (i = 0; i < HIDDEN_SIZE; i++) {
        ...
        for (j = 0; j < OUTPUT_SIZE; j++) {
            ...
            lastHiddenDeltas[j] += lastOutputDelta * rnn->outputLayer.weightsState[j];
        }
        hiddenDeltas[i] *= sigmoidDerivative(hiddenOutputs[i]);
        lastHiddenDeltas[i] *= sigmoidDerivative(rnn->hiddenLayer.lastState[i]);
    }
    ...
    for (i = 0; i < HIDDEN_SIZE; i++) {
        ...
        rnn->hiddenLayer.weightsState[i] += learningRate * lastHiddenDeltas[i] * rnn->hiddenLayer.state[i];
    }
    rnn->lastError = error;
...
void testRNN(const RNN *rnn, double **features, double *target, int numSamplesTest, FILE *file) {
    ...
    int numSamplesValid = numSamplesTest;
    ...
    double mape = ((sumPercentageError / numSamplesValid) * 100);
